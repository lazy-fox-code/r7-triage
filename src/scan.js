...
// Перекрытие окон
// ---------------
// Соседние окна перекрываются на `overlapDays`. Документация строгость
// сравнения дат не фиксирует. По исходникам 115 (ext-messages.js) сравнение
// включительное и с точностью до секунды: `msg.dateInSeconds * 1000` против
// `fromDate` / `toDate`. Посуточной грубости там нет, но секунды усечены, а
// письмо с точной датой границы при строгом сравнении не вернул бы ни один
// из двух запросов. Перекрытие стоит только дедупликации, которая уже есть,
// поэтому оставлено как страховка от смены семантики в следующих версиях.
//
// Цена окна. Любой `messages.query` в 115 перебирает папку целиком
// (`folder.messages`) и лишь затем фильтрует по дате. Окно сужает выдачу,
// но не работу клиента: запросов к большой папке должно быть мало.
//
    ...
    for (const hdr of list) {
      ...
      if (locs) {
        ...
        if (stored) {
          ...
          // Обогащение не нашло письмо на прежнем месте, а оно здесь: его
          // перенесли. Возвращаем в очередь сразу, не дожидаясь повтора неудач.
          if (stored.enriched === -1 && stored.enrichError === "not-found") {
            stored.enriched = 0;
          }
          rows.push(stored);
          locs.push(fkey);
